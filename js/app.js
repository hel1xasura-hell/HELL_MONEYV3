...
function viewBank() {
  ...
  return `
    ...
    <div class="glass p-4 mb-4 glow-violet">
      <div class="flex items-center gap-2 mb-1">
        <span class="badge badge-violet">Super Admin &amp; Admin</span>
        <h3 class="font-display font-semibold text-sm">Give Loan</h3>
      </div>
      <p class="text-xs text-low mb-3">Issues a loan from the Central Bank to a player, at the interest rate set above. Tracked separately from a direct Send Money transfer.</p>
      <form id="give-loan-form" class="grid sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-start">
        <input class="field" name="username" placeholder="@telegram_username" required>
        <input class="field" name="amount" type="number" min="1" step="1" placeholder="Amount ($)" required>
        ...
        <button class="btn btn-primary" type="submit">Issue Loan</button>
        <input class="field sm:col-span-4" name="note" placeholder="Note (optional) — shown in the audit log">
      </form>
      <div id="give-loan-error" class="hidden text-xs text-ember mt-2"></div>
    </div>
    <div class="glass p-4 mb-4">
      <h3 class="font-display font-semibold text-sm mb-3">Outstanding Loans</h3>
      <div class="overflow-x-auto"><table>
        <thead><tr><th>Borrower</th><th>Amount</th><th>Interest</th><th>Term</th><th>Due</th><th>Status</th></tr></thead>
        <tbody id="loans-tbody">${loansRows(MOCK.activeLoans)}</tbody>
      </table></div>
    </div>
    <div class="glass p-4">
...
function loansRows(list) {
  return list.map((l) => `
    <tr>
      <td class="strong">${l.user}</td>
      <td class="text-gold">${fmtMoney(l.amount)}</td>
      <td>${l.interest}%</td>
      <td>${l.termMonths} mo</td>
      <td class="text-low">${l.due}</td>
      <td><span class="badge ${l.status === 'overdue' ? 'badge-ember' : 'badge-good'}">${l.status}</span></td>
    </tr>`).join('');
}
...
afterRender.bank = () => {
  ...
  const loanForm = document.getElementById('give-loan-form');
  if (loanForm) loanForm.addEventListener('submit', (e) => {
    ...
    const note = fd.get('note').trim();
    const err = document.getElementById('give-loan-error');
    err.classList.add('hidden');
    if (!username.startsWith('@')) username = '@' + username;
    if (!amount || amount <= 0) {
      err.textContent = 'Enter a valid loan amount.';
      err.classList.remove('hidden');
      return;
    }
    // HellMoneyAPI.bank.giveLoan(username, amount, term, note) once the
    // Railway backend exposes POST /bank/give-loan.
    MOCK.activeLoans.unshift({
      id: Date.now(), user: username, amount, interest: MOCK.bank.interestRate,
      termMonths: term, issued: 'today', due: `in ${term} mo`, status: 'active',
    });
    document.getElementById('loans-tbody').innerHTML = loansRows(MOCK.activeLoans);
    loanForm.reset();
    toast(`Issued ${fmtMoney(amount)} loan to ${username} at ${MOCK.bank.interestRate}% (mock).`);
  });
};
